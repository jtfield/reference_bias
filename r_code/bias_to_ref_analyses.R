...
unambig_errors_sum = sum(compare_data$diffs_to_true)
print(unambig_errors_sum)

unambig_errors_matching_ref_sum = sum(diffs_to_true_matching_ref)
...
total_bases_compared = 2845 * 2328289
...
errors_subset_df$diffs_to_true_not_matching_ref = (errors_subset_df$diffs_to_true - errors_subset_df$diffs_to_true_matching_ref)

print(errors_subset_df)

ref_errors_df = errors_subset_df[ , c('diffs_to_true_matching_ref', 'diffs_to_true_not_matching_ref')]

sum_ref_errors = sum(ref_errors_df$diffs_to_true_matching_ref)
avg_ref_errors = mean(ref_errors_df$diffs_to_true_matching_ref)
min_ref_errors = min(ref_errors_df$diffs_to_true_matching_ref)
max_ref_errors = max(ref_errors_df$diffs_to_true_matching_ref)
...
sum_non_ref_errors = sum(ref_errors_df$diffs_to_true_not_matching_ref)
avg_non_ref_errors = mean(ref_errors_df$diffs_to_true_not_matching_ref)
min_non_ref_errors = min(ref_errors_df$diffs_to_true_not_matching_ref)
max_non_ref_errors = max(ref_errors_df$diffs_to_true_not_matching_ref)
print(sum_non_ref_errors)
print(avg_non_ref_errors)
print(min_non_ref_errors)
print(max_non_ref_errors)

ref_errors_density = hist(ref_errors_df$diffs_to_true_matching_ref)
#plot(ref_errors_density)

non_ref_errors_desnity = hist(ref_errors_df$diffs_to_true_not_matching_ref)
